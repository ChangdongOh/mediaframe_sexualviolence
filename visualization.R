...
space_world = df$Topic4 + df$Topic6 + df$Topic9 + df$Topic25 + df$Topic40 + df$Topic58 + 
  df$Topic62 + df$Topic74 + df$Topic82 + df$Topic91 + df$Topic92 + df$Topic93
space_organization = df$Topic8 + df$Topic38 + df$Topic71
space_politics = df$Topic12 + df$Topic37 + df$Topic46 + df$Topic54 + df$Topic77 + df$Topic94
space_art = df$Topic13 + df$Topic17 + df$Topic53 + df$Topic68
space_sports_entertainment = df$Topic3 + df$Topic34 + df$Topic79
space_polmil = df$Topic16 + df$Topic47
space_work_emotion = df$Topic19 + df$Topic39 + df$Topic50 
space_digital = df$Topic21
...
df = cbind(solution_punish, solution_str, sens_depiction, empathy_nar, objective_desc,
           mental_disorder, acquaintance_repeat, intimate_partner, space_art, space_digital,
           space_organization, space_politics, space_polmil, space_professional, 
           space_public, space_school, space_sports_entertainment, space_work_emotion,
           space_world, df[96:98])

sub_data = df %>% filter(grepl('', press))
sub_data = sub_data[1:20] %>%
  group_by(month) %>% 
  summarise_all(funs(sum))
sub_data[2:20] = sub_data[2:20]/rowSums(sub_data[2:20])
...
ggplot(data = d, 
  ...
  labs(list(title = paste0('시간 변화에 따른 토픽 규모 변화'),
            x = '시기(연도 및 월)', 
            y = '토픽이 차지하는 비율')) +
...
ggsave('solutions.png', width = 30, height=24, dpi = 300, units="cm")
sub_data
...
ggplot(data = d, 
  ...
  annotate(geom = 'text', 
           ...
           y = -max(d$value)*0.06, size = 3,
           label = seq(2002, 2018, 1))
ggsave('images.png', width = 30, height=24, dpi = 300, units="cm")
...
ggplot(data = d, 
  ...
  annotate(geom = 'text', 
           ...
           y = -max(d$value)*0.06, size = 3,
           label = seq(2002, 2018, 1))
ggsave('stereotypes_criminal.png', width = 30, height=24, dpi = 300, units="cm")
